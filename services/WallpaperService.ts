...
export async function captureAndSave(
  ...
  if (!captureRef || !MediaLibrary) {
    return { success: false, message: 'Installation requise : expo-media-library react-native-view-shot' };
  }
  const granted = await requestGalleryPermission();
  if (!granted) return { success: false, message: 'Permission galerie photos refusée.' };

  try {
    const { width, height } = FORMATS[format];
    const uri = await captureRef(viewRef, { format: 'jpg', quality: 0.95, width, height });
    await MediaLibrary.saveToLibraryAsync(uri);
    return { success: true, message: `${FORMATS[format].label} sauvegardé dans votre galerie ☸` };
  } catch {
...
export function showSaveResult(result: { success: boolean; message: string }) {
  Alert.alert(result.success ? '☸ Sauvegardé' : 'Erreur', result.message, [{ text: 'OK' }]);
}
