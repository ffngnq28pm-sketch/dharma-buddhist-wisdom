  ...
  return (
    <TouchableOpacity
      style={[
        styles.container,
        {
          backgroundColor: nearLimit ? 'rgba(198,124,43,0.07)' : colors.bgSection,
          borderColor: nearLimit ? 'rgba(198,124,43,0.35)' : colors.border,
        },
      ]}
      onPress={onPress}
      activeOpacity={0.82}
    >
      <Animated.View style={[styles.dot, { opacity: glow, backgroundColor: nearLimit ? '#C67C2B' : colors.textMuted }]} />

      <View style={styles.textBlock}>
        {nearLimit ? (
          <Text style={[styles.label, { color: '#C67C2B' }]}>
            {remaining === 0
          ...
          <Text style={[styles.label, { color: colors.textMuted }]}>
            Contenu Premium ☸ · Sagesse {currentIndex + 1} sur {FREE_CARD_LIMIT}
          </Text>
        )}
      </View>

      <View style={[styles.pill, { backgroundColor: 'rgba(198,124,43,0.12)', borderColor: 'rgba(198,124,43,0.3)' }]}>
        <Sparkles size={10} color="#C67C2B" />
