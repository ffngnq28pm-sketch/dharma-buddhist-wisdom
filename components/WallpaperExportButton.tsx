...
export function WallpaperExportButton({ targetRef, format = 'wallpaper', compact = false }: Props) {
  ...
  if (compact) {
    return (
      <TouchableOpacity
        style={[styles.compact, { borderColor: 'rgba(198,124,43,0.35)', backgroundColor: 'rgba(198,124,43,0.07)' }]}
        onPress={handlePress}
        disabled={loading}
        activeOpacity={0.75}
      >
        {loading ? <ActivityIndicator size="small" color={accent} /> : <ImageIcon size={16} color={accent} />}
  ...
  return (
    <View ref={internalRef}>
      <TouchableOpacity
        style={[styles.btn, { backgroundColor: 'rgba(198,124,43,0.1)', borderColor: 'rgba(198,124,43,0.3)' }]}
        onPress={handlePress}
        disabled={loading}
        activeOpacity={0.8}
      >
...
const styles = StyleSheet.create({
  btn: { flexDirection: 'row', alignItems: 'center', gap: 14, borderRadius: 14, borderWidth: 1, padding: 16, marginVertical: 6 },
  btnTitle: { fontFamily: 'Cinzel_700Bold', fontSize: 14, marginBottom: 2 },
  btnSub: { fontFamily: 'Lato_400Regular', fontSize: 11 },
  compact: { flexDirection: 'row', alignItems: 'center', gap: 6, borderRadius: 10, borderWidth: 1, paddingHorizontal: 12, paddingVertical: 7 },
