...
import { BUDDHIST_TEACHERS, BuddhistTeacher } from '@/data/teachers';
...
export default function TeachersScreen() {
  ...
  const [selected, setSelected] = useState<BuddhistTeacher | null>(null);

  const renderItem = ({ item }: { item: BuddhistTeacher }) => (
    <TouchableOpacity
    ...
    >
      <View style={styles.cardImageWrap}>
        ...
        <View style={[styles.leftAccentBar, { backgroundColor: item.color }]} />
      </View>

      <View style={styles.cardBody}>
        <View style={styles.fieldRow}>
          {item.tradition.split(' — ').slice(0, 2).map((f) => (
        ...
        <Text style={[styles.namePali, { color: colors.textMuted }]}>{item.paliName}</Text>
            ...
            <ScrollView contentContainerStyle={styles.detailScroll} showsVerticalScrollIndicator={false}>
              <View style={styles.detailNameBlock}>
                <View style={styles.fieldRowDetail}>
                  {selected.tradition.split(' — ').map((f) => (
                ...
                <Text style={[styles.detailNamePali, { color: colors.textMuted }]}>{selected.paliName}</Text>
...
const styles = StyleSheet.create({
  ...
  namePali: { fontFamily: 'Cinzel_400Regular', fontSize: 11, fontStyle: 'italic', marginBottom: 2 },
  ...
  detailNamePali: { fontFamily: 'Cinzel_400Regular', fontSize: 12, fontStyle: 'italic', marginBottom: 4, textAlign: 'center' },
