...
const TIPS = [
  ...
  { size: 'large' as const,  emoji: '🙏', label: 'Mois de développement', price: '25,00€', desc: 'Que le Dharma vous protège' },
];

const MISSION_POINTS = [
  { emoji: '✨', text: 'Préserver et transmettre la sagesse millénaire du Bouddha et des maîtres bouddhistes' },
  { emoji: '📱', text: 'Rendre accessible la pratique authentique face aux distractions modernes' },
  { emoji: '🌍', text: 'Promouvoir la compassion et la pleine conscience dans un univers digital superficiel' },
  { emoji: '📚', text: 'Continuer d\'enrichir le contenu avec de nouveaux enseignements, sutras et pratiques' },
];
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.header}>
            <View style={[styles.iconBadge, { backgroundColor: 'rgba(198,124,43,0.1)' }]}>
              <Heart size={30} color={accent} fill="rgba(198,124,43,0.25)" />
            </View>
          ...
          <View style={[styles.verseBox, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            <Text style={[styles.versePali, { color: accent }]}>Dānaṃ deti, sīlaṃ rakkhati.</Text>
            <Text style={[styles.verseFr, { color: colors.textMuted }]}>
              Il donne, il garde la discipline. — Anguttara Nikaya
            </Text>
          </View>

          {donated && (
            <View style={[styles.successBox, { backgroundColor: 'rgba(198,124,43,0.08)', borderColor: 'rgba(198,124,43,0.3)' }]}>
              <Sparkles size={16} color={accent} />
              <Text style={[styles.successText, { color: accent }]}>
                ☸ Que le Dharma vous protège — Merci pour votre don de {donated}
              </Text>
            </View>
          )}
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    active && { backgroundColor: 'rgba(198,124,43,0.07)' },
                  ]}
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.75}
          >
            <Star size={16} color={accent} fill="rgba(198,124,43,0.2)" />
          ...
          <TouchableOpacity
            ...
            onPress={() => Linking.openURL('mailto:support@dharma-app.fr?subject=Dharma%20-%20Retour%20utilisateur')}
            activeOpacity={0.8}
          >
            <Mail size={18} color={accent} />
            <View style={{ flex: 1 }}>
              <Text style={[styles.contactTitle, { color: colors.textPrimary }]}>Écrire à l'équipe</Text>
              <Text style={[styles.contactSub, { color: colors.textMuted }]}>support@dharma-app.fr</Text>
            </View>
            <ChevronLeft size={16} color={colors.textMuted} style={{ transform: [{ rotate: '180deg' }] }} />
          </TouchableOpacity>

          <View style={styles.footer}>
            <Text style={[styles.footerWheel, { color: 'rgba(198,124,43,0.25)' }]}>☸</Text>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>
              Que ce projet contribue à la paix et à l'éveil de tous les êtres
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  versePali: { fontFamily: 'Cinzel_400Regular', fontSize: 15, textAlign: 'center', fontStyle: 'italic' },
