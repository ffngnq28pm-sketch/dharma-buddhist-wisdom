...
const VISUAL_THEMES: VisualTheme[] = [
  { key: 'dark',  label: 'Nuit du Dharma',    desc: 'Fond sombre — défaut',             preview: ['#080C09', '#0D1410'] },
  { key: 'light', label: 'Aube dorée',         desc: 'Parchemin ivoire et safran',        preview: ['#F9F2E8', '#EDE0CC'] },
  { key: 'sepia', label: 'Parchemin ancien',   desc: 'Terre cuite — scriptorium zen',     preview: ['#1A1005', '#2E1F0C'] },
];
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      ...
      <SafeAreaView style={{ flex: 1 }}>
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          ...
          {!isPremium ? (
            <TouchableOpacity
              style={[styles.premiumBanner, { borderColor: 'rgba(198,124,43,0.3)' }]}
              onPress={() => setPremiumVisible(true)}
              activeOpacity={0.85}
            >
              <LinearGradient colors={['#0D1A10', '#080C09']} style={styles.premiumGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
                <View style={styles.premiumLeft}>
                  <View style={styles.starBadge}><Star size={16} color="#C67C2B" fill="#C67C2B" /></View>
                  <View>
                    <Text style={styles.premiumTitle}>Dharma Premium</Text>
            ...
            <View style={[styles.premiumActive, { borderColor: 'rgba(198,124,43,0.25)', backgroundColor: 'rgba(198,124,43,0.08)' }]}>
              <Star size={16} color="#C67C2B" fill="#C67C2B" />
              <Text style={[styles.premiumActiveText, { color: accent }]}>Dharma Premium actif ☸</Text>
            </View>
          )}
          ...
          <View style={[styles.section, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            ...
            <View style={styles.nameSub}>
              <Text style={[styles.nameSubText, { color: colors.textMuted }]}>
                Utilisé pour : "Bonsoir {profile.firstName || 'prénom'}, voici votre sagesse du jour"
              </Text>
            </View>
          </View>
          ...
          <View style={[styles.section, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            <View style={styles.sectionHeader}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>Thème de pratique</Text>
            ...
            <View style={styles.focusGrid}>
              {FOCUS_THEMES.map((t) => (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={styles.focusBtnIcon}>{FOCUS_THEME_ICONS[t]}</Text>
          ...
          <View style={[styles.section, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            <View style={styles.row}>
              ...
              <Switch
                ...
                trackColor={{ false: colors.bgSection, true: 'rgba(198,124,43,0.4)' }}
            ...
            {profile.notifEnabled && (
              <>
                ...
                <View style={[styles.sectionHeader, { paddingBottom: 6 }]}>
                  ...
                  <Text style={[styles.presetTitle, { color: colors.textMuted }]}>Heure de méditation</Text>
                </View>
                {NOTIF_PRESETS.map((p) => (
                  <TouchableOpacity
                    key={p.key}
              ...
              <View style={styles.rowLeft}>
                ...
                <View>
                  <Text style={[styles.rowTitle, { color: colors.textSecondary }]}>
                    Mode nuit {isNightMode ? '🌙' : ''}{!isPremium ? ' ✦ Premium' : ''}
                  </Text>
                  <Text style={[styles.rowDesc, { color: colors.textMuted }]}>
                    {isNightMode ? 'Actif — filtre ambré' : 'Fond sombre, filtre ambré doux'}
                  </Text>
                </View>
              </View>
              <Switch
          ...
          <View style={[styles.section, { backgroundColor: colors.bgSection, borderColor: colors.border }]}>
            <TouchableOpacity style={styles.row} activeOpacity={0.75} onPress={() => Linking.openURL('https://ffngnq28pm-sketch.github.io/dharma-buddhist-wisdom/privacy.html')}>
              <View style={styles.rowLeft}>
            ...
            <View style={[styles.separator, { backgroundColor: colors.border }]} />
            <TouchableOpacity style={styles.row} activeOpacity={0.75} onPress={() => Linking.openURL('https://ffngnq28pm-sketch.github.io/dharma-buddhist-wisdom/terms.html')}>
              <View style={styles.rowLeft}>
                <Info size={18} color={accent} />
            ...
            <TouchableOpacity style={styles.row} activeOpacity={0.75}>
              <View style={styles.rowLeft}>
                <Info size={18} color={accent} />
                <Text style={[styles.rowTitle, { color: colors.textSecondary }]}>À propos de Dharma</Text>
              </View>
              <ChevronRight size={16} color={colors.textMuted} />
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.footer}>
            <Text style={[styles.footerWheel, { color: colors.textMuted }]}>☸</Text>
            <Text style={[styles.footerTitle, { color: colors.textMuted }]}>Dharma · Éveil</Text>
            <Text style={[styles.footerText, { color: colors.textMuted }]}>Version 1.0.0</Text>
            <Text style={[styles.footerTagline, { color: colors.textMuted }]}>Sagesse bouddhiste — chaque jour.</Text>
          </View>
        </ScrollView>
      </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  starBadge: { width: 38, height: 38, borderRadius: 19, backgroundColor: 'rgba(198,124,43,0.15)', alignItems: 'center', justifyContent: 'center' },
  premiumTitle: { fontFamily: 'Lato_700Bold', fontSize: 15, color: '#F5EDE0', marginBottom: 2 },
  premiumDesc: { fontFamily: 'Lato_400Regular', fontSize: 12, color: '#A0927A' },
  premiumCta: { backgroundColor: '#C67C2B', borderRadius: 10, paddingHorizontal: 16, paddingVertical: 8 },
  premiumCtaText: { fontFamily: 'Lato_700Bold', fontSize: 13, color: '#080C09' },
  ...
  saveBtnText: { fontFamily: 'Lato_700Bold', fontSize: 13, color: '#080C09' },
  ...
  focusBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 12, paddingVertical: 8, borderRadius: 20, borderWidth: 1 },
  focusBtnIcon: { fontSize: 14 },
  focusBtnText: { fontFamily: 'Lato_400Regular', fontSize: 12 },
  ...
  activeCheckText: { fontFamily: 'Lato_700Bold', fontSize: 12, color: '#080C09' },
